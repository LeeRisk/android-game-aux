...
class ClientActor(args: Seq[String]) extends FSM[Status, Data] with FsmHelper[Status, Data] {
  ...
  def statusMap() = {
    ...
    args.foldLeft(map) {
      case (acc, "war-2-6") => map + (War -> (() => actorOf(Props(new WarAreaTwoSix(warNum)))))
      case (acc, "war-6-4") => map + (War -> (() => actorOf(Props(new WarAreaSixActor(warNum)))))
      case (acc, "wdj")     => map + (War -> (() => actorOf(Props(new WdjActor(warNum)))))
      case (acc, "dismiss") => map + (Dismissed -> (() => actorOf(Props(new nyhx.fsm.DismissedActor))))
      case _                => map
    }
  }
